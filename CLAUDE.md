...
- **part_requests**: request_code(unique), buyer_id(fk), part_type(enum used|new|both), maker, car_model, vin(nullable), mfg_date(nullable), oem_part_number(nullable), part_name, reference_url(nullable), memo(nullable), status(enum §5), selected_response_id(fk nullable), confirmed_vendor_id(fk nullable), cost_price(nullable), applied_rate(nullable), applied_min_fee(nullable), buyer_price(nullable), shipping_method(enum dhl|vehicle|container nullable), shipping_fee(nullable), timestamps, softDeletes
- **vendor_responses**: part_request_id(fk), vendor_id(fk), cost_price, quality_rank(enum S|A|B|C nullable), lead_time(enum), comment(text), is_no_stock(bool), timestamps

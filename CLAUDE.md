...
- **settings**: key, value, type  (holds margin_rate, margin_min_fee, admin_sender_email. `shipping_fee_vehicle`/`shipping_fee_container` are gone — shipping moved to a rule-based weight model, see `shipping_weight_brackets` below and §14 Phase 4)
- **shipping_weight_brackets**: upper_kg(decimal nullable — null marks the top/catch-all "and above" bracket, for heavy items like engines), fee(int, yen), order(int — app-managed sort key, not DB-unique, since renumbering it must be able to pass through a transient collision mid-update) -- admin-configurable weight → fee lookup for the v1 rule-based shipping model (§14 Phase 4). `ShippingCalculator` picks the smallest bracket whose upper_kg covers the given weight, ordered by `order`.
...
- **Phase 4 — Payments & ordering**: buyer checkout (shipping selection) → payment → payment gate → `ordered_to_vendor` + `procurement_failed` path → buyer invoice + vendor invoice. Built as six slices, each shipped and fully tested before the next starts:
  1. **Payment abstraction** — gateway-agnostic `PaymentGateway` interface (shaped loosely around Stripe PaymentIntents, generic enough to swap providers) + a `StubPaymentGateway` that assumes success, clearly marked dev-only. Active gateway chosen via config (`PAYMENT_GATEWAY`) — the stub must never be the production default. `payments` table. Calling code depends on the interface only; swapping in a real gateway later must require zero changes to callers. **Implemented — `StripePaymentGateway` is now the production gateway** (raw stripe-php PaymentIntents SDK, not Cashier): `charge()` only ever *creates* a PaymentIntent and returns; it never marks a payment confirmed itself. The ONLY thing ever allowed to do that is `ConfirmStripePaymentAction`, called exclusively from `App\Http\Controllers\StripeWebhookController` after verifying Stripe's webhook signature — never the browser, never a redirect landing back on the site. `CheckoutAction` needed zero changes to support this (proving slice 1's own promise); see CONVENTIONS.md's "Stripe: confirmation is webhook-only" section for the full design, including the deliberate `paid`-status-before-`confirmed`-payment window this creates and why the §6.3 gate stays safe regardless. Stripe.js/Elements client-side card collection + confirmation is deferred to a later UI slice — this is the backend (gateway + webhook + idempotency + tests) only.
  2. **Shipping addresses.**
  3. **Checkout — 有償 (paid) flow.** Shipping is rule-based (v1, weight-only): a `shipping_weight_brackets` table (upper_kg/fee/order, admin-configurable, extending high enough to cover heavy items like engines) auto-calculates the fee from the presented vendor response's own `weight_kg`, computed at **quote-presentation time** (`PresentQuoteAction`) — not at checkout. The admin sees the calculated fee when presenting and may override it with a **required** reason, stored and activity-logged on the `presented_quotes` row itself — admin-internal, never shown to the buyer. The fee (auto or overridden) is snapshotted onto `presented_quotes.shipping_fee` the moment it's presented — same never-recompute-later discipline as pricing (§6.2) — and flows through unchanged to checkout, which has no shipping-method selector or fee logic of its own any more (the vehicle/container fixed-fee picker originally built in this slice is removed). Size/dimensions are deferred past v1; DHL is a separate method entirely (slice 4).
  4. **DHL shipping** — a second shipping method, deferred: a DHL Express (MyDHL) Rating-API auto-calculation is the plausible long-term approach, but it requires the *client's own* DHL Express business account, and whether they have one is still unconfirmed. Not yet designed.
  5. **無償 (free) flow** — confirmed spec: admin sets a free flag when *presenting* the quote (not at checkout); buyer price is ¥0 and buyer checkout skips payment entirely for that request; vendor cost is still recorded (margin/reporting integrity holds even at ¥0 to the buyer); no mixing paid and free line items on the same request; no integration with any old/legacy system for this flow. **Implemented** — the concrete mechanism:
     - `presented_quotes.is_free` (bool, default false) is set by `PresentQuoteAction`'s new `$isFree` param, purely admin-discretionary. When true, `buyer_price` and `shipping_fee` on that same row are forced to 0 regardless of `PricingService`/`ShippingCalculator` — `cost_price`/`applied_rate`/`applied_min_fee` stay the real, normally-computed figures, so the admin's own accounting still shows what's actually owed to the vendor. `$isFree` and `$shippingFeeOverride` are mutually exclusive (nothing to override on a fee forced to zero — `PresentQuoteNotAllowedException::cannotOverrideFreeShipping()`). A request cannot mix free and paid presented quotes: presenting a quote whose `is_free` disagrees with any already-presented quote on the same request throws `PresentQuoteNotAllowedException::mixedFreeAndPaidNotAllowed()`.
     - `part_requests.is_free` (bool, default false) is copied from the selected presented quote by `SelectQuoteAction`, the same way `buyer_price`/`shipping_fee` already are — never derived from `buyer_price === 0`, which would be an implicit, easy-to-break proxy.
     - **The payment gate (§6.3) stays completely unchanged.** A free order satisfies it the same way a real charge would: `ConfirmFreeOrderAction` (the mirror of `CheckoutAction`) creates a genuine `Payment` row — `amount: 0`, `status: Confirmed`, `gateway: 'waived'` — so `ConfirmOrderToVendorAction`'s existing check (`status === Paid && payments()->where('status', Confirmed)->exists()`) opens without any special-casing. No new `RequestStatus` case was needed; a free order reaches `paid` exactly like a real one.
     - `CheckoutAction` refuses to run against an `is_free` request (`CheckoutNotAllowedException::isFreeRequest()`) and `ConfirmFreeOrderAction` refuses to run against a non-free one (`FreeOrderNotAllowedException::notFree()`) — each is the other's mirror image, never a shared action with a flag (§8).
     - Buyer UX (confirmed with the client): checkout is skipped entirely, but the buyer still confirms/picks a shipping address — a lightweight address-only step reusing the existing address-selection UI minus the payment section, since a free order still needs to ship somewhere and the buyer's default address isn't necessarily right for this particular shipment.
     - Isolation unchanged: buyer never sees `cost_price`/vendor identity on a free order any more than a paid one — nothing about the isolation column-allowlisting changes for this flow.
  6. **Order-confirm to vendor** — delivery method + company address.
  7. **Invoicing** — buyer invoice + vendor invoice.

  Key decisions spanning the phase:
  - **Gateway**: confirmed as Stripe (currency JPY) — `PAYMENT_GATEWAY=stripe` selects `StripePaymentGateway` in production; `stub` remains for local dev and the test suite only (never bindable in production, see `PaymentServiceProvider`).

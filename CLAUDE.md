...
- **Phase 4 — Payments & ordering**: buyer checkout (shipping selection) → payment → payment gate → `ordered_to_vendor` + `procurement_failed` path → buyer invoice + vendor invoice. Built as six slices, each shipped and fully tested before the next starts:
  ...
  3. **Checkout — 有償 (paid) flow** — vehicle/container only. DHL is deliberately excluded from this slice's checkout (see slice 4) — no admin screen exists to set `part_requests.shipping_fee` for it, and rushing one in risked the wrong shape (see slice 4's note).
  4. **DHL shipping** — its own slice, not part of 3, because of a real ordering problem: a DHL fee depends on the buyer's *destination address*, which the buyer only picks *during* checkout. "The admin enters the DHL fee at quote time" (this section's shipping-model paragraph below, written before Slice 2/3 existed) assumed the fee could be known before checkout — it can't, not once the address itself is chosen at checkout. The real flow is closer to: buyer reaches checkout, picks/confirms their address → admin is shown that address and enters/confirms the DHL fee → buyer sees the fee and pays. That needs deliberate design (does checkout pause mid-flow for the admin? is there a quote-refresh step? what does the buyer see while waiting?) rather than being squeezed into slice 3. Not yet designed.
  ...
  **This is also where the shipping model gets finalized, not before**: `shipping_fee_vehicle`/`shipping_fee_container` (§7 `settings`) are provisional fixed fees pending client confirmation, priced from Settings at checkout time and then snapshotted onto `part_requests.shipping_fee`/`shipping_method` — never recomputed afterward, same discipline as pricing (§6.2). DHL is realistically per-request, not a global fixed number — see slice 4 above for why "the admin enters the fee" is more involved than it first sounds once the address is chosen at checkout, not before it. A DHL Express (MyDHL) Rating-API auto-calculation is a plausible later enhancement, but it requires the *client's own* DHL Express business account; whether they have one is an open question to confirm with the client before scoping any DHL API work.

...
| Buyer | James Carter | Global Auto Parts Ltd | `buyer1@demo.test` | Verified, member code `BYR-000006` |
| Buyer | Fatima Al-Sayed | Pacific Rim Motors | `buyer2@demo.test` | Verified, member code `BYR-000007` |
| Buyer | Liam O'Connor | Southern Cross Auto Imports | `buyer3@demo.test` | **Unverified** — shows the badge/resend flow, member code `BYR-000008` |
...
4. Back on the list, click **New vendor** and fill in the form. On
   ...
   **Copy**, tick "I've saved this password", then **Done**. That
   password is never shown again and never emailed (CLAUDE.md §14).
...
2. Open that row's **Actions** menu and click **Resend verification
   ...
   inbox at `http://127.0.0.1:1080` if you've pointed `.env` at it).
...
4. You land in the app normally afterward, still showing the
   verification banner (admin-created accounts aren't pre-verified).
